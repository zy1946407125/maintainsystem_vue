<template>
    <div>
        <div style="display: flex;flex-direction: row;height: 50px">
            ...
            <el-button @click="exportExcel()" icon="el-icon-search" type="primary">导出表格</el-button>
        </div>
        <div id="father">
...
<script>
    ...
    export default {
        ...
        methods: {
            exportExcel() {
                const sTime = this.timestampToTime(this.valueTime[0])
                const eTime = this.timestampToTime(this.valueTime[1])
                const type = this.type
                const status = this.status
                const fileName = sTime + "__" + eTime + "__" + type + "__" + status + ".xlsx"
